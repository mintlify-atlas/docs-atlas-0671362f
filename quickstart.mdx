---
title: "Get started with OxygenDispatch in 5 minutes"
sidebarTitle: "Quickstart"
description: "Clone, configure, and run OxygenDispatch locally. Follow this guide to register your first batch and dispatch oxygen cylinders."
---
...
<Tip>
  Want to skip the manual setup steps? The `composer.json` defines a `setup` script that chains the entire installation sequence in one command: `composer install`, copy `.env.example`, `php artisan key:generate`, `php artisan migrate --force`, `npm install`, and `npm run build`. Just run:
