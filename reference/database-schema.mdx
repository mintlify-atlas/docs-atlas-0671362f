...
description: "Complete database schema reference for OxygenDispatch — all tables, columns with types and constraints, and foreign key relationships derived from the migration files."
---
...
The central table — one row per physical cylinder. Uses a UUID primary key. `product_id`, `serial_prefix`, `serial_number`, and `updated_at` were added by subsequent migrations.

| Column | Type | Constraints | Notes |
|---|---|---|---|
| `id` | `uuid` (char 36) | PK | Auto-generated UUID |
| `serial` | `varchar(120)` | NOT NULL, UNIQUE | Full serial string |
| `serial_prefix` | `varchar(10)` | nullable | Alphabetic prefix of the serial |
| `serial_number` | `bigint unsigned` | nullable | Numeric portion of the serial |
| `batch_id` | `bigint unsigned` | NOT NULL, FK → `batches.id` CASCADE DELETE | |
| `product_id` | `bigint unsigned` | nullable, FK → `catalog_products.id` | |
