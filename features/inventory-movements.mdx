...
description: "OxygenDispatch records every tank action — intake, transfer, dispatch, and status change — as an inventory movement with timestamp, user, and reference document."
---
...
| Technical status changed manually | `CAMBIO_ESTADO_TECNICO` (4) | Notes describe the old → new status |
| Tank approved during technical review | `CAMBIO_ESTADO_TECNICO` (4) | `from_area_id = Recepción`, `to_area_id = Productos aprobados` |
| Tank rejected during technical review | `CAMBIO_ESTADO_TECNICO` (4) | `to_area_id = Rechazos/devoluciones` |
